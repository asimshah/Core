...
To create such an empty database, run the following command in powershell with the current directory set to C:\devroot\Fastnet.Core.UserAccounts:
...
The schema can be modified in the normal way by changing UserAccountDb. To keep it simple, delete any current database existing sql server db called userAccounts and the
...
DO NOT use the package mabager as this requires a separate startup project.
